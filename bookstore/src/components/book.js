...
import { editOpen } from "../redux/bookEdit/actions";
import BookDelete from "../redux/book/thunk/bookDelete";

const Book = ({ book }) => {
  const { id, name, author, featured, thumbnail, price, rating } = book;
  const dispatch = useDispatch();

  let rate = [0, 0, 0, 0, 0];
  for (var i = 0; i < rating; i++) rate[i] = 1;

  const handleEdit = () => {
    dispatch(editOpen(id, book));
  };
